 ...
 * 为所有优惠券类型提供通用功能
 */
export class MCouponBlock extends MBlock {
  // 静态属性声明
  static prefix?: string;
  static couponType?: string;
  ...
  _handleClick(event: MouseEvent) {
    ...
    // 获取组件类型
    const type = (this.constructor as typeof MCouponBlock).couponType || this._config.default_type;
    ...
    const clickEvent = new CustomEvent('coupon-click', {
      ...
      detail: {
        ...
        updateCoupon: (selectedValue: any) => this._updateCouponData(selectedValue)
      }
    });
   ...
   * 更新优惠券数据
   */
  _updateCouponData(selectedValue: any) {
    ...
    const default_text = config.default_text || (this.constructor as typeof MCouponBlock).couponType + '优惠券';
    ...
    const updateEvent = new CustomEvent('component-updated', {
      ...
      detail: {
        id: this.id,
        type: (this.constructor as typeof MCouponBlock).couponType,
        data: this.getData()
      }
    });
  ...
  _notifyEditorContentChanged() {
    ...
    if (editorElement) {
      // 调用编辑器的更新方法
      if (typeof (editorElement as any)._updateValueFromContent === 'function') {
        setTimeout(() => (editorElement as any)._updateValueFromContent(), 0);
      } else {
   ...
   * @param data 完整数据对象
   */
  _processDisplayText(defaultText: string, value: string, data: any): string {
    const config = this._config || {};
    ...
    if (config.show_process && typeof config.show_process === 'function') {
      try {
        // 准备组件配置和上下文信息
        const constructor = this.constructor as typeof MCouponBlock;
        const componentInfo = {
          // 静态属性
          tagName: constructor.tagName || 'unknown',
          prefix: constructor.prefix || this.getPrefix(),
          defaultIcon: constructor.defaultIcon || '🏷️',
          styleClass: constructor.styleClass || 'default-style',
          couponType: constructor.couponType || config.default_type || 'coupon',
  ...
  _render() {
    ...
    if (isRestoreComponent && data.originalFormat) {
      ...
      // 使用标准格式化显示
      const type = constructor.couponType || config.default_type || '优惠券';
      displayText = `${prefix ? `{${prefix}-${data.value}}` : `${type}: ${data.value}`}`;
      console.log('使用标准格式显示:', displayText);
    } else {
      // 使用常规显示文本
      displayText = data.displayText || config.default_text || constructor.couponType + '优惠券';
      console.log('使用常规显示文本:', displayText);
    }

    // 判断是使用图片还是图标
    const iconClass = constructor.iconClass || 'default-icon';
    const defaultIcon = constructor.defaultIcon || '🏷️';
    ...
    // 获取组件的CSS类名
    const styleClass = constructor.styleClass || 'default-style';
   ...
   * 设置组件为回显组件
   */
  setRestoreStatus(isRestore: boolean) {
    // 确保数据对象存在
   ...
   * 设置为回显模式并展示原始格式
   */
  setAsRestoreComponent(originalFormat: string) {
    // 确保数据和配置对象存在
    if (!this._data) this._data = {};
    if (!this._config) this._config = {};
    ...
    this.innerHTML = `
        <div class="block-content ${constructor.styleClass || 'default-style'} is-restore">
            <span class="block-icon ${iconClass}">${defaultIcon}</span>
...
if (typeof window !== 'undefined' && !window.customElements.get('m-coupon-block')) {
