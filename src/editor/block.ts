 ...
 * 为所有自定义块提供公共功能
 */
export class MBlock extends HTMLElement {
  _data: any = {};
  _prefix: string = '';
  _template: string = '${prefix}_${value}';
  _config: any = {
    label_key: 'name',
    value_key: 'code',
    icon_url: '',
  ...
  _isVueEnvironment: boolean;
  static tagName: any;

  // 添加静态属性和方法
  static register(editor: any) {
    if (editor && typeof editor.registerComponent === 'function') {
      editor.registerComponent(this.tagName.toLowerCase(), {
        constructor: this,
        prefix: (this as any).prefix || ''
      });
    } else {
      console.error('无法注册组件：编辑器对象无效或缺少注册方法');
    }
  }
  ...
  constructor() {
    ...
    // Vue环境检测
    this._isVueEnvironment = (this.constructor as typeof MBlock).isVueEnvironment();
  }
  ...
  /**
   * 使用模板生成最终值
   * @param template 模板字符串，例如 "${prefix}_${value}"
   */
  getValueWithTemplate(template: string): string {
   ...
   * 包装渲染操作，确保与Vue的兼容性
   */
  _safeRender(renderFn: Function) {
    if (this._isVueEnvironment) {
      // 在Vue环境中，暂时移除数据响应式监听
      const vueInstance = (this as any).__vue__;
...
if (typeof window !== 'undefined' && !window.customElements.get('m-block')) {
